...
(function ($) {
  ...
  $.fn.bart = function (method) {
    ...
    if (($.type(args) == "number") | ($.type(args) == "array"))
      args = { b: args };
    if ($.type(args.b) == "number") {
      // number given
    ...
    } else if ($.type(args.b) == "array") {
      // array of different balloon settings
      var bopts,
        cnt = 0;
    ...
    for (var i = 0; i < bs.length; i++) {
      if (opts.frmids_pumps[i] === undefined)
        opts.frmids_pumps[i] = "BARTpumps" + (i + 1);
      if (opts.frmids_exploded[i] === undefined)
        opts.frmids_exploded[i] = "BARTexploded" + (i + 1);
      if (opts.frmids_time[i] === undefined)
        opts.frmids_time[i] = "BARTtime" + (i + 1);
    }
    ...
    if (opts.sounds == true) {
      ...
      } else if (new Audio().canPlayType("audio/x-wav")) {
        // wav for the rest
    ...
    balloon.prototype.inflate = function (canvas) {
      ...
      var centerY =
        canvas.height() / 2 -
        (this.radius + this.radius * this.height_factor) / 4;
      ...
      // bottom Y of balloon
      var balloonBottomY =
        centerY + this.radius + this.radius * this.height_factor;
    ...
    balloon.prototype.save = function () {
      ...
      for (var i = 1, t = 0; i < this.time.length; i++)
      ...
      r =
        r + [this.id, this.pumps, this.exploded * 1, t].join(opts.separator[0]);
        ...
        .on("click.bart", function (e) {
          ...
          if (opts.showcurrentearned) {
            canvas.setLayer("curearn", {
              text: opts.txt_current_earned + new Number(bal.earned).toFixed(2),
            });
          }
          if (opts.showpopprob) {
            canvas.setLayer("popprob", {
              text:
                opts.txt_prob_explosion +
                "\n\n" +
                new Number(Math.round(10000 / bal.popseq.length) / 100).toFixed(
                  2
                ) +
                "%",
            });
          ...
          if (opts.showpumpsused) {
            canvas.setLayer("pumuse", {
              text:
                ...
                "\n\n" +
                Math.round(
                  ((bal.popprob - bal.popseq.length) / bal.popprob) * 100
                ) +
                "%",
            });
          }
        ...
        .on("click.bart", function (e) {
          ...
          if (opts.showballooncount) {
            canvas.setLayer("balnum", {
              text: opts.txt_balloon_number + (balcnt + 1) + " / " + bs.length,
            });
          }
          if (opts.showpopprob) {
            canvas.setLayer("popprob", {
              text:
                ...
                "\n\n" +
                new Number(
                  Math.round(10000 / bs[balcnt].popprob) / 100
                ).toFixed(2) +
                "%",
            });
          }
